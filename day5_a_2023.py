...
def parse(seed, map):
    ...
    for r, v in enumerate(map):
        #r = row
        #v = value
        source, dest, length = v.split()
        # find dest for seed
        if int(seed) >= int(source) and int(seed) <= (int(source) + int(length)):
            diff = int(dest) - int(source)
            valueI = int(seed) + diff
            value = str(valueI)
            break
    return value

def parse_numbers(seed, s2s, s2f, f2w, w2l, l2t, t2h, h2l):
    #parse this seed to find location
...
def process_the_data(theData):
    ...
    for seed in seeds_list:
        #seed to soil
        if (i % 2) != 0: 
            location = parse_numbers(seed, s2s, s2f, f2w, w2l, l2t, t2h, h2l)
            location_list.append(int(location))
            previous_seed = seed
        else:
            # increase seed by 1 'seed' times!
            seed_range = int(seed)
            ii = 1
            while ii < seed_range:
                seed = str(int(previous_seed) + ii)
                location = parse_numbers(seed, s2s, s2f, f2w, w2l, l2t, t2h, h2l)
                location_list.append(int(location))
