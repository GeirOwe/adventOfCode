...
def add_signal_to_vertical(x1, y1, x2, y2, signalGrid):
    #prepare
    row = y1
    column = x1
    #moving up or down
    ...
    # no of signals to add for this line
    noOfSign = x1 - x2
    if noOfSign < 0:
        noOfSign = noOfSign * -1
    i = 0
    #increase number in grid for all cells in this line
    while i <= noOfSign:
        signalGrid[row, column] += 1
        #then decide what cell to step to next in the grid
        if step == 1:
            row += 1
            column += 1
        else:
...
def process_the_data(linesData, signalGrid):
    ...
    # ifra linesData[1] t.o.m linesData[3]
        ...
        #add signals to a column if x1 = x2
            ...
            #vertical line
            signalGrid = add_signal_to_vertical(x1, y1, x2, y2, signalGrid) 
...
def check_if_vertical(x1, y1, x2, y2):
    vertical = False
    # all combinations of 45 degrees lines
    if (x1 + x2 == y1 + y2):
        vertical = True
    if (x1 - x2 == y1 - y2): 
        vertical = True    
...
def get_coord(row):
    #format from input to coordinates. from "5,5 -> 8,2" to x1, y1, x2, y2
    temp = row.split("->")
    temp1 = temp[0].split(",")
    temp2 = temp[1].split(",")
...
def optimize_the_data(theData):
    ...
    for row in theData:
        ...
        #test if vertical line -> part2
        vertical_line = check_if_vertical(x1, y1, x2, y2)
        if x1 == x2 or y1 == y2 or vertical_line:
    ...
    rowsInGrid += 1
    colsInGrid += 1
    #linesdata contains all lines in dataset that qualifies to be plotted to the grid
    linesData = np.array(coords, dtype = "int").reshape(noOfRows, 4)
    #signalgrid is the total grid where we are to plot the lines
