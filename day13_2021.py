...
def fold_the_map(the_map, fold_instr, maxCol):
    #part 1 - only first folding instruction
    split = fold_instr[0].split("=")
    #find split position
    ...
    if "y" in split[0]: 
        fold = "y"
        folded_map = fold_horizontally(the_map,fold, splitPos, maxCol)
    else: 
        fold = "x"
        folded_map = fold_vertically(the_map,fold, splitPos, maxCol)
...
def start_the_engine():
    ...
    # Next, you need to 
    print('\nHow many dots are visible after completing just the first fold -> ', valueX,'\n')
    return 
