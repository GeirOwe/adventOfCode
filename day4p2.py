...
def check_all_fields(df):
    #errPass = df[(df['byr'] >= 1920) & (df['byr'] <= 2002)] #byr (Birth Year) - four digits; at least 1920 and at most 2002.
    errPass = df
    return errPass
...
def check_pass_for_err(df):
    ...
    wrongPass = df[df['ecl'].isnull() | df['pid'].isnull() | df['eyr'].isnull() | df['hcl'].isnull() | df['byr'].isnull() | df['iyr'].isnull() | df['hgt'].isnull()]
    
    #the part two challenge
    moreErrPass = check_all_fields(wrongPass)

    invalidPassports = len(moreErrPass)
