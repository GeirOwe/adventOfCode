...
def process_the_data(grid):
    ...
    while i < noSteps:
        ...
        i += 1
        # check if it is all zeroes in the grid
        nonZeroes = (grid != 0 ).sum()
        if nonZeroes == 0:  #all zeroes
            print("kun nuller i grid -> antall iterasjon utført er -> ", i)
            i += noSteps
    return valueX
